# -*- coding: utf-8 -*-
"""
Created on Sat Dec 28 07:31:46 2024
...
median_agg = df_agg_diff[df_agg_diff['Video publish time'] >= metric_date_12mo].select_dtypes(include=['float64', 'int64']).median()
...
if add_sidebar == 'Aggregate Metrics':
    ...
    metric_medians6mo = df_agg_metrics.select_dtypes(include=['float64', 'int64']).loc[df_agg_metrics['Video publish time'] >= metric_date_6mo].median()
    metric_medians12mo = df_agg_metrics.select_dtypes(include=['float64', 'int64']).loc[df_agg_metrics['Video publish time'] >= metric_date_12mo].median()
    ...
    for i in metric_medians6mo.index:
        with columns[count]:
            if pd.isna(metric_medians6mo[i]) or pd.isna(metric_medians12mo[i]):
                st.write(f"Skipping {i} due to missing data")
                continue

            if metric_medians12mo[i] == 0:
                st.write(f"Skipping {i} due to division by zero")
                continue
    ...
    df_agg_diff_final = df_agg_diff.loc[:,['Video title','Publish_date','Views','Likes','Subscribers','Shares','Comments added','RPM(USD)','Average % viewed','Avg_duration_sec', 'Engagement_ratio','Views / sub gained']]

    numeric_cols = df_agg_diff_final.select_dtypes(include=['float64', 'int64']).columns
    df_agg_numeric_lst = numeric_cols.tolist()
